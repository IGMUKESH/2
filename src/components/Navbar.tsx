...
import { LogIn, LayoutDashboard, Quote, User, LogOut, ChevronDown, Info, FileText, Shield, Settings, Sun, Moon } from 'lucide-react';
...
export const Navbar: React.FC = () => {
  ...
  const { selectedCategory, setSelectedCategory, categories } = useCategories();
            ...
            <div className="grid grid-cols-4 sm:flex sm:flex-wrap gap-1.5 justify-center w-full">
              {(isExpanded ? categories : categories.slice(0, 7)).map((cat, index) => (
                <button
                  ...
                  className={`px-1 py-1.5 rounded-lg transition-all text-[12px] font-bold tracking-wide border font-laila text-center truncate ${selectedCategory === cat ? 'bg-[#F27D26] text-white border-[#F27D26] shadow-lg shadow-[#F27D26]/20' : 'bg-gray-100 dark:bg-[#151619] text-gray-500 dark:text-[#8E9299] border-gray-200 dark:border-white/5 hover:border-gray-300 dark:hover:border-white/10'}`}
                >
                  {cat}
                </button>
              ))}
