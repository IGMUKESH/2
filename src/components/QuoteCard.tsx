...
import { Quote as QuoteIcon, Share2, Heart, MessageCircle, Download, MessageSquare, User, Edit, Camera } from 'lucide-react';
import { motion } from 'framer-motion';
import { db, auth, doc, setDoc, deleteDoc, onSnapshot, updateDoc, increment, handleFirestoreError, collection, query, getDoc } from '../firebase';
...
export const QuoteCard: React.FC<QuoteCardProps> = ({ quote }) => {
  ...
  const handleOpenEditModal = (mode: 'full' | 'photo') => {
  ...
  useEffect(() => {
    ...
    const userRef = doc(db, 'users', quote.createdBy);
    const unsubscribe = onSnapshot(userRef, (doc) => {
      if (doc.exists()) {
        setUserData(doc.data());
      }
    }, (error) => {
      handleFirestoreError(error, OperationType.GET, `users/${quote.createdBy}`);
    });
    return unsubscribe;
  }, [quote.createdBy]);
    ...
    >
      {quote.imageUrl && (
        <div className="relative w-full overflow-hidden bg-gray-100 dark:bg-black flex flex-col justify-center">
              ...
              <>
                {!imageLoaded && (
                  <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/40 backdrop-blur-sm z-10">
                    ...
                    <p className="text-[10px] text-[#8E9299] font-bold uppercase tracking-widest">Loading Image...</p>
                  </div>
                )}
                <img 
                  ref={imageRef}
                  ...
                  className={`w-full max-h-[calc(100dvh-280px)] object-contain bg-black transition-opacity duration-700 ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}
          ...
          <div className="absolute top-4 right-4 flex flex-col gap-2 z-30 opacity-0 group-hover:opacity-100 transition-all duration-300">
            <button 
              onClick={handleShare}
              className="p-3 bg-blue-500 rounded-2xl text-white shadow-lg hover:scale-110 transition-transform"
              title="Share"
            >
              <Share2 size={20} />
            </button>
            ...
            {quote.imageUrl && (
              <button 
                ...
                className="p-3 bg-[#F27D26] rounded-2xl text-white shadow-lg hover:scale-110 transition-transform disabled:opacity-50"
      ...
      <div className="p-4 md:p-6 flex flex-col justify-center items-center text-center bg-white dark:bg-[#000000]">
        ...
        <div className="flex items-center gap-2 pt-3 w-full">
          <button 
            onClick={handleShare}
            className="flex-1 flex items-center justify-center py-3 rounded-2xl transition-all font-bold bg-blue-500 text-white hover:bg-blue-600 shadow-md"
            title="Share"
          >
            <Share2 size={20} />
          </button>
          ...
          <button 
            onClick={handleDownload}
            disabled={isDownloading}
            className="flex-1 flex items-center justify-center py-3 rounded-2xl transition-all font-bold bg-white dark:bg-black text-black dark:text-white shadow-sm hover:bg-gray-50 dark:hover:bg-white/5"
          >
            {isDownloading ? (
